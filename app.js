(function() {
  ...
  function navHtml() {
    ...
    return `<nav>${tabs.map(t =>
      `<button class="${currentTab === t.key ? 'active' : ''}" data-tab="${t.key}">${t.label}</button>`
    ).join('')}</nav>`;
  }
  ...
  function renderHistory() {
    ...
    if (filtered.length === 0) {
      html += `<p style="text-align:center;color:#888;padding:20px;">Нет записей</p>`;
    }
  ...
  function renderStats() {
    ...
    // 🔥 TOP SIZES (most important)
    html += `<h3>🔥 Топ размеров (по количеству)</h3>`;
    html += `<ul class="top-list">${topSizes.map(([k, v]) =>
      `<li><span>${k}</span><span>${v}</span></li>`).join('') || '<li>—</li>'}</ul>`;

    // Chart (14 days)
    html += `<h3>📈 Динамика по дням (последние 14 дней)</h3><canvas id="chart" width="400" height="180"></canvas>`;

    // Export / Import
    html += `<hr><div class="actions">
      <button class="btn secondary" id="exportCSV">📥 Экспорт CSV</button>
      <button class="btn secondary" id="exportXLSX">📥 Экспорт XLSX</button>
